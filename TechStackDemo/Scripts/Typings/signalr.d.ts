...
interface SignalR {
   ...
   // createHubProxy(hubName: string): SignalR;
    ...
    //MED IT CHANGE - THESE ARE ONLY ON HubConnection
    ...
    starting(handler: (status:any) => void ): SignalR;
    received(handler: (data: any) => void ): SignalR;
    error(handler: (error: string) => void): SignalR;
    stateChanged(handler: (oldState: number, newState: number) => void): SignalR;
    stateChanged(handler: (change: SignalRStateChange) => void ): SignalR;
    disconnected(handler: () => void ): SignalR;
    connectionSlow(handler: (status:any) => void ): SignalR;
    sending(handler: () => void ): SignalR;
    reconnecting(handler: (status:any) => void): SignalR;
    reconnected(handler: (status:any) => void): SignalR;
}
...
interface HubConnection extends SignalR {
    ...
    //MED IT CHANGE - MOVED HERE FROM SignalR
    start(): JQueryPromise<void>;
    start(callback: () => void ): JQueryPromise<void>;
    start(settings: ConnectionSettings): JQueryPromise<void>;
    start(settings: ConnectionSettings, callback: () => void): JQueryPromise<void>;
