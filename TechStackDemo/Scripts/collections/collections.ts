...
module collections {
    ...
    */
    ...
    export class Dictionary<K, V>{
         ...
         */
        constructor(toStrFunction?: (key: K) => string, initialData?: {key: K, value: V}[] | V[], initialDataKeyExtractor?: (value:V)=>K) {
            this.table = {};
            this.nElements = 0;
            this.toStr = toStrFunction || collections.defaultToString;

            if (initialDataKeyExtractor && initialData) {
                (initialData as V[]).forEach(d=> {
                    this.setValue(initialDataKeyExtractor(d), d);
                });
            } else if (initialData) {
                (initialData as { key: K, value: V }[]).forEach(keyValue=> this.setValue(keyValue.key, keyValue.value));
            }
        }
        ...
        */
        ...
        map(callback: (key: K, value: V) => any): Array<any> {
            var array = new Array();
        ...
        */
        ...
        /**
         * Retrieves the value from the dictionary for the given key. If not existing then 
         * the value is created, stored in the dictionary and then returned
         */
        getOrAddValue(key: K, factory: (keyValue: K) => V, ifExists?: (value: V, key: K)=> void) {
    ...
    export class Set<T>{
         ...
         * unique string must be provided.
         * @param initialData optional - used to populate the set upon creation
         */
        constructor(toStringFunction?: (item: T) => string, initialData?: T[]) {
            this.dictionary = new Dictionary<T, any>(toStringFunction, initialData, initialData && (x=>x));            
        }
